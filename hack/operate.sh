...
function publish_bundle() {
    ...
    popd &>/dev/null
    error_run "Building bundle manifests" 'kustomize build --load-restrictor none config/manifests | operator-sdk generate bundle --overwrite --version $VERSION --channels "$CHANNELS"' || return 1
